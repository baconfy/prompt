...
@section('content')
    ...
    <div class="space-y-2">
        @foreach ($versions as $version)
            ...
            <details class="group overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900" @if ($isLatest) open @endif>
                <summary class="flex cursor-pointer items-center justify-between gap-3 px-4 py-3 [&::-webkit-details-marker]:hidden">
                    <div class="flex flex-1 items-center gap-3">
                        <svg class="size-4 shrink-0 text-slate-400 transition-transform group-open:rotate-90 dark:text-slate-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <div class="flex shrink-0 items-center gap-2">
                        @unless ($isLatest)
                            <form method="POST" action="{{ route('prompts.versions.restore', ['prompt' => $prompt, 'version' => $version->id]) }}" onsubmit="return confirm('Restore v{{ $version->version_number }}? A new version with this content will be created on top.');">
                                @csrf
                                <button type="submit" class="rounded-md border border-slate-300 bg-white px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800">
                                    Restore
                                </button>
                            </form>
                        @endunless

                        <form method="POST" action="{{ route('prompts.versions.destroy', ['prompt' => $prompt, 'version' => $version->id]) }}" onsubmit="return confirm('Delete v{{ $version->version_number }} permanently? This cannot be undone.');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="rounded-md border border-rose-200 bg-white px-3 py-1.5 text-xs font-medium text-rose-700 hover:bg-rose-50 dark:border-rose-900/50 dark:bg-slate-900 dark:text-rose-300 dark:hover:bg-rose-900/30">
                                Delete
                            </button>
                        </form>
                    </div>
                ...
                <div>
                    @if ($isLatest)
                        <pre class="border-t border-slate-200 bg-slate-50 px-4 py-3 font-mono text-xs leading-relaxed whitespace-pre-wrap text-slate-800 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200">{{ $version->content }}</pre>
                    @else
